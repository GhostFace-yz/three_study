...
/**
 * 环境：主光源（通用模式，按需增删）。
 * 原项目无灯光（MeshBasicMaterial 不受光照影响），此光源不影响画面，
 * 供后续使用受光材质（MeshStandardMaterial / 自定义 shader）时直接可用。
...
export default class Environment {
  ...
  updateMaterials() {
    this.scene.traverse((child) => {
      if (child instanceof THREE.Mesh && child.material instanceof THREE.RawShaderMaterial) {
